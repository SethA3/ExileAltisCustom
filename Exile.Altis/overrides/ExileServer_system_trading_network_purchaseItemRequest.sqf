...
private["_isVIP","_discountRate","_sessionID","_parameters","_itemClassName","_quantity","_containerType","_containerNetID","_playerObject","_vehicleObject","_salesPrice","_playerMoney","_playerRespect","_quality","_requiredRespect","_logging","_traderLog","_responseCode"];
...
try 
{
	...
	//Added by Raingod to provision VIP discounts
	_isVIP = _playerObject getVariable["isVIP",false];
	_discountRate = getNumber(missionConfigFile >> "CfgVIP" >> "discountRate");
	
	if (_isVIP && _discountRate > 0) then
	{
		_salesPrice = round(_salesPrice - (_salesPrice * _discountRate));
	};
